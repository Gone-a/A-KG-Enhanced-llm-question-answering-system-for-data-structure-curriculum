...
def main(cfg):
    ...
    print(f'Label mapping: {vocab.id2label}')
    ...
    for i in tqdm(range(0, len(sorted_data), batch_size), desc="推理进度"):
        ...
        for j, data in enumerate(batch):
            ...
            result = {"sentence": data["sentence"], "head": "", "tail": "", "head_offset": "", "tail_offset": "", "head_type": "", "tail_type": ""}
            entity_found = False
            ...
            if len(decode_entities) > 0:
                ...
                # 处理头实体
                head_indexes, head_type_id = head_entity
                result["head"] = ''.join([words[idx] for idx in head_indexes])
                result["head_offset"] = str(min(head_indexes))
                result["head_type"] = vocab.id2label.get(head_type_id, 'CON')  # 使用词汇表映射类型ID到类型标签
                entity_found = True
                
                # 处理尾实体（如果存在且位置在头实体后）
                if tail_entity:
                    tail_indexes, tail_type_id = tail_entity
                    if min(tail_indexes) > min(head_indexes):  # 确保尾实体在头实体之后
                        ...
                        result["tail_type"] = vocab.id2label.get(tail_type_id, 'CON')  # 使用词汇表映射类型ID到类型标签
                    else:
                        # 如果尾实体在头实体前，交换它们
                        ...
                        result["head_type"], result["tail_type"] = vocab.id2label.get(tail_type_id, 'CON'), vocab.id2label.get(head_type_id, 'CON')
    ...
    # 保存结果到指定目录
    output_dir = os.path.join(utils.get_original_cwd(), 'DeepKE', 'example', 're', 'standard', 'data', 'predict')
    ...
    for r in all_results:
        if r.get('head') and r.get('tail'):
            # 使用预测的实体类型，如果为空则使用默认值CON
            if not r.get('head_type'):
                r['head_type'] = 'CON'
            if not r.get('tail_type'):
                r['tail_type'] = 'CON'
            filtered_results.append(r)
    
    # 保存为CSV格式
    output_path = os.path.join(output_dir, 'predict.csv')
    with open(output_path, 'w', encoding='utf-8', newline='') as f:
        ...
        for result in filtered_results:
            writer.writerow({
                ...
                'tail_type': result['tail_type']
            })
    print(f"已保存预测结果到 {output_path} (有效实体: {len(filtered_results)}/{len(all_results)})")
    
    # 保存无实体句子
    no_entity_path = os.path.join(output_dir, 'no_entity_sentences.txt')
