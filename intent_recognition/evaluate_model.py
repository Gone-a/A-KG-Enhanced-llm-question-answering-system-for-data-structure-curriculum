...
# --- LLM NER 提示词（One-shot，数据结构课程场景） ---
ner_system = """你是一名数据结构课程的助教。你的任务是从给定段落中抽取与数据结构与算法相关的命名实体，并以JSON格式返回。
请仅抽取和课程相关的实体，如：数据结构名称（如“二叉树”“红黑树”“哈希表”“并查集”“线段树”“跳表”“Trie”）、算法名称（如“堆排序”“快速排序”“Dijkstra”“Kruskal”“Prim”“拓扑排序”）、复杂度术语（如“时间复杂度”“空间复杂度”“O(n log n)”）、存储与图论术语（如“邻接表”“邻接矩阵”“最短路”“最小生成树”）等。
返回格式示例：{"named_entities": ["实体1", "实体2", ...]}
"""

one_shot_ner_paragraph = """在这节数据结构课上，我们比较了二叉树与红黑树在插入操作上的性能，并讨论了堆排序与快速排序的时间复杂度差异：堆排序通常为O(n log n)，而快速排序在平均情况下也是O(n log n)。随后我们介绍了图的存储方式，包括邻接表和邻接矩阵，并通过Kruskal与Prim算法求最小生成树，最后还讲了Dijkstra算法用于单源最短路的求解，以及并查集在连通性判断中的应用。"""

one_shot_ner_output = """{"named_entities": [
    "二叉树", "红黑树", "插入", "堆排序", "快速排序",
    "时间复杂度", "O(n log n)", "邻接表", "邻接矩阵",
    "Kruskal", "Prim", "最小生成树", "Dijkstra", "单源最短路",
    "并查集", "连通性"
]}"""
